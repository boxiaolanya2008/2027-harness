...
interface Pattern {
  ...
  suggestion: string
  test: RegExp
  matchedText?: (command: string, match: RegExpMatchArray) => string
}
...
export function assessShellCommand(command: string): ShellRiskAssessment | null {
  ...
  for (const pattern of ALL_PATTERNS) {
    ...
    if (match) {
      const matched = pattern.matchedText ? pattern.matchedText(value, match) : match[0].trim().slice(0, 120)
      return {
        level: pattern.level,
...
export function isCriticalCommand(command: string): boolean {
  return assessShellCommand(command)?.level === 'critical'
}

export function isRiskyCommand(command: string): boolean {
  const result = assessShellCommand(command)
  return result?.level === 'risky' || result?.level === 'critical'
}
